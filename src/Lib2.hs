...
parseQuery input =
  case parseString input of
    ...
    Right (command, remaining) ->
      if command == "add_concert_tickets_seller"
                        ...
                        then case parseConcert remaining of
                          ...
                          if command == "remove_ticket"
                            then case parseConcert remaining of
                              ...
                              if command == "sell_ticket"
                                then case parseTicket remaining of
                                  Left err -> Left err
                                  Right (ticket, _) ->
                                    Right (SellTicket ticket)
                                else
                                  if command == "return_ticket"
                                    then case parseTicket remaining of
                                      Left err -> Left err
                                      Right (ticket, _) ->
                                        Right (ReturnTicket ticket)
                                    else
                                      if command == "change_concert_information"
                                        then case parseConcert remaining of
                                          Left err -> Left err
                                          ...
                                          if command == "check_available_tickets"
                                            then case parseConcert remaining of
                                              Left err -> Left err
                                              Right (concert, _) ->
...
updateTicketAvailability st ticket newStatus =
  let updatedConcerts =
      ...
      ticketAlready =
        any
          ( \c ->
              any (\t -> ticketId t == ticketId ticket && availability t == newStatus) (tickets c)
          )
          (concerts st)
          ...
          case parseDate restArtist of
            ...
            Right (matchedDate, restDate) ->
              if null restDate
                ...
                else case parseChar ',' restDate of
                  Left err -> Left err
                  Right (_, remaining) ->
